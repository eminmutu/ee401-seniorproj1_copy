...
loadscript VoltmeterFunctions
    function StartVoltmeterBurst(count)
        ...
        -- 2. VOLTMETER SOURCE CONFIGURATION
        ...
        -- 4. START EXECUTION
        print("Status: Starting measurement...")
...
class KeithleyVoltmeterApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Keithley 2602B High-Speed Voltmeter")
    ...
    def _setup_ui(self):
        ...
        self.start_btn = ttk.Button(ctrl_frame, text="1. Start Measurement", command=self.start_measurement, state="disabled")
        self.start_btn.pack(side="left", padx=10)
        
        self.fetch_btn = ttk.Button(ctrl_frame, text="2. Fetch & Plot Results", command=self.fetch_data, state="disabled")
        ...
        self.data_text = scrolledtext.ScrolledText(data_frame, width=25, height=20, state='disabled', font=("Consolas", 10))
    ...
    def start_measurement(self):
        ...
        try:
            ...
            self.inst.write(f"StartVoltmeterBurst({count})")
            self.status_var.set(f"Measurement of {count} points started...")
        except ValueError:
            messagebox.showerror("Input Error", "Count must be an integer.")
        except Exception as e:
    ...
    def fetch_data(self):
        ...
        try:
            ...
            while True:
                try:
                    ...
                    if "DataStart" in line:
                        started = True
                        continue # Don't add the tag to content
                    if "DataEnd" in line:
                        break
                    if "Error" in line:
            ...
            if not raw_content:
                self.status_var.set("No data found between tags.")
                return

            # --- PARSING FIX ---
            # Instead of regex, we split by comma. 
            # This correctly handles scientific notation (e.g., 1.234e-09).
            # We filter out empty strings resulting from trailing commas or newlines.
            voltages = []
            for item in raw_content.replace('\n', ',').split(','):
                ...
                if item:
                    try:
                        ...
                        pass # Ignore non-numeric garbage if any
            ...
            for i, val in enumerate(voltages):
                # Using scientific notation in the display to be precise
    ...
    def _on_closing(self):
        ...
        sys.exit(0)         # Force kills the process
