...
class SDG1020PulseGui:
	def __init__(self, root: tk.Tk) -> None:
		...
		burst = tk.LabelFrame(root, text="Burst", padx=12, pady=10)
		burst.grid(row=1, column=0, sticky="ew", pady=(0, 6), padx=12)
		for col in range(5):
			burst.grid_columnconfigure(col, weight=1)

		self.burst_enable = tk.BooleanVar(value=False)
		tk.Checkbutton(burst, text="Enable burst", variable=self.burst_enable).grid(
			row=0, column=0, sticky="w"
		)

		tk.Label(burst, text="Cycles (NCYC):").grid(row=0, column=1, sticky="e")
		self.burst_cycles_var = tk.StringVar(value="5")
		tk.Entry(burst, textvariable=self.burst_cycles_var, width=8).grid(
			row=0, column=2, sticky="w"
		)

		tk.Label(burst, text="Mode:").grid(row=1, column=0, sticky="e", pady=(6, 0))
		self.burst_mode_var = tk.StringVar(value="NCYC")
		tk.OptionMenu(burst, self.burst_mode_var, "NCYC", "GATE").grid(
			row=1, column=1, sticky="w", pady=(6, 0)
		)

		tk.Label(burst, text="Trigger Source:").grid(row=1, column=2, sticky="e", pady=(6, 0))
		self.burst_source_var = tk.StringVar(value="INT")
		tk.OptionMenu(burst, self.burst_source_var, "INT", "EXT", "MAN").grid(
			row=1, column=3, sticky="w", pady=(6, 0)
		)

		tk.Label(burst, text="Burst Period (s / SI)").grid(
			row=2, column=0, sticky="e", pady=(6, 0)
		)
		self.burst_period_var = tk.StringVar(value="")
		tk.Entry(burst, textvariable=self.burst_period_var, width=12).grid(
			row=2, column=1, sticky="w", pady=(6, 0)
		)

		tk.Label(burst, text="Burst Delay (s / SI)").grid(
			row=2, column=2, sticky="e", pady=(6, 0)
		)
		self.burst_delay_var = tk.StringVar(value="")
		tk.Entry(burst, textvariable=self.burst_delay_var, width=12).grid(
	...
	def apply_pulse(self):
		...
		inst.write(cmd)
		burst_msg = self._configure_burst(inst, period)
		status = (
	...
	def _configure_burst(self, inst, base_period):
		prefix = f"C{CHANNEL}:BTWV"
		if not self.burst_enable.get():
			inst.write(f"{prefix} STATE,OFF")
			return "Burst disabled"
		mode = self.burst_mode_var.get().strip().upper() or "NCYC"
		if mode not in {"NCYC", "GATE"}:
			raise ValueError("Burst mode must be NCYC or GATE")
		source = self.burst_source_var.get().strip().upper() or "INT"
		if source not in {"INT", "EXT", "MAN"}:
			raise ValueError("Trigger source must be INT, EXT, or MAN")
		cycles = None
		if mode == "NCYC":
			cycles_text = self.burst_cycles_var.get().strip()
			if not cycles_text:
				raise ValueError("Cycles required when mode is NCYC")
			cycles = int(float(cycles_text))
			if cycles <= 0:
				raise ValueError("Burst cycles must be positive")
		period_text = self.burst_period_var.get().strip()
		burst_period = None
		if mode == "NCYC" and source != "EXT":
			if period_text:
				burst_period = self._parse_time(period_text)
			else:
				burst_period = base_period
			if burst_period <= 0:
				raise ValueError("Burst period must be positive")
		delay_text = self.burst_delay_var.get().strip()
		burst_delay = None
		if delay_text:
			burst_delay = self._parse_time(delay_text)
			if burst_delay < 0:
				raise ValueError("Burst delay must be non-negative")
		inst.write(f"{prefix} STATE,ON")
		inst.write(f"{prefix} GATE_NCYC,{mode}")
		inst.write(f"{prefix} TRSR,{source}")
		if cycles is not None:
			inst.write(f"{prefix} TIME,{cycles}")
		if burst_period is not None:
			inst.write(f"{prefix} PRD,{burst_period:.9g}")
		if burst_delay is not None and mode == "NCYC":
			inst.write(f"{prefix} DLAY,{burst_delay:.9g}")
		if mode == "GATE":
			return f"Burst GATE, src {source}"
		return f"Burst {cycles} cyc, src {source}"
