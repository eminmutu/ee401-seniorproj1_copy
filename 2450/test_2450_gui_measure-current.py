"""Tkinter GUI for controlling measure-current.tsp."""
...
SCRIPT_NAME = "AmmeterScript"
SCRIPT_FILE = pathlib.Path(__file__).with_name("measure-current.tsp")


class AmmeterGUI:
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("2450 Ammeter")
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(frame, text="Range (A, blank=auto):").grid(column=0, row=2, sticky="w")
        ...
        plot_frame = ttk.LabelFrame(self.root, text="Current Samples")
    ...
    def disconnect(self) -> None:
        if self.inst is not None:
            try:
                self.inst.write("pcall(ammeter_output_off)")
    ...
    def measure(self) -> None:
        ...
        try:
            response = self.inst.query(
                f"print(measure_current({samples}, {range_arg}, {nplc_arg}))"
            ).strip()
            self._log(f"Result: {response}")

            buffer_values = self.inst.query("printbuffer(1, defbuffer1.n, defbuffer1)").strip()
            currents = self._parse_buffer(buffer_values)
            if currents:
                self._log("Buffer currents (A):")
                self._log(", ".join(self._format_si(i) for i in currents))
    ...
    def _update_plot(self, currents: list[float]) -> None:
        self.ax.clear()
        scale, prefix = self._best_si_scale(currents)
        self.ax.set_xlabel("Sample")
        self.ax.set_ylabel(f"Current ({prefix}A)")
        self.ax.grid(True, linestyle="--", alpha=0.6)
        if currents:
            x_vals = list(range(1, len(currents) + 1))
            scaled = [i / scale for i in currents]
            self.ax.plot(
                x_vals,
                scaled,
                marker="o",
            ...
            self.ax.fill_between(x_vals, scaled, color="tab:red", alpha=0.1)
            self.ax.set_xlim(0.5, len(currents) + 0.5)
        self.ax.set_title("Measured Currents")
        self.canvas.draw_idle()

    def _best_si_scale(self, values: list[float]) -> tuple[float, str]:
        if not values:
            return 1.0, ""
        ...
        for scale, prefix in scales:
            if max_abs < scale * 1000:
                return scale, prefix
        return 1.0, ""

    def _format_si(self, value: float) -> str:
        scale, prefix = self._best_si_scale([value])
        return f"{value / scale:.6f} {prefix}A"
