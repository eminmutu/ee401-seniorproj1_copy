"""GUI to drive run_fast_external_trigger() in the current-measurement TSP script.

This interface mirrors the voltage-oriented GUI but targets the
`2450_triggered_measure_current.tsp` helper so operators can tweak
parameters, launch acquisitions, and inspect the captured currents along a
...
TSP_SOURCE = ROOT / "2450_triggered_measure_current.tsp"
SCRIPT_NAME = "FastExternalTriggerCurrent"
DEFAULT_ADDRESS = "TCPIP0::169.254.188.69::5025::SOCKET"
EDGE_CHOICES = ("rising", "falling", "either")
READ_DRAIN_TIMEOUT_MS = 750


class ExternalTriggerCurrentGUI:
    """Tkinter GUI that wraps run_fast_external_trigger() for current capture."""

    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("2450 External Trigger – Current Capture")
        ...
        self.compliance_var = tk.StringVar(value="0.01")
        self.source_range_var = tk.StringVar(value="2")
        self.measure_range_var = tk.StringVar(value="0.01")
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(params, text="Compliance (A)").grid(column=0, row=1, sticky="w", padx=(0, 6), pady=(8, 0))
        ttk.Entry(params, textvariable=self.compliance_var, width=12).grid(column=1, row=1, sticky="w", pady=(8, 0))

        ttk.Label(params, text="Source range (V)").grid(column=0, row=2, sticky="w", padx=(0, 6), pady=(8, 0))
        ttk.Entry(params, textvariable=self.source_range_var, width=12).grid(column=1, row=2, sticky="w", pady=(8, 0))

        ttk.Label(params, text="Measure range (A)").grid(column=0, row=3, sticky="w", padx=(0, 6), pady=(8, 0))
        ...
        ttk.Label(plot_container, text="Captured currents", font=("Segoe UI", 12, "bold")).grid(column=0, row=0, sticky="w")
        plot_frame = ttk.LabelFrame(plot_container, text="Current vs. sample")
    ...
    def start_measurement(self) -> None:
        ...
        try:
            ...
            comp_arg = self._format_number(self.compliance_var.get(), allow_nil=True)
        ...
        command = (
            ...
            f"{count_arg}, {comp_arg}, {src_range_arg}, {meas_range_arg}, {trig_line}, {edge_arg}, {nplc_arg}"
            ")"
        )
        ...
        self._log(
            "Waiting: count=%s, compliance=%s A, Vrng=%s V, Irng=%s A, NPLC=%s, line=%d (%s edge)"
            % (
                count_arg,
                comp_arg,
                src_range_arg,
    ...
    def _async_finish(
        ...
        *,
        ...
        def finalize() -> None:
            ...
            currents = [v for _, v in data]
            self.status_var.set("Measurement complete")
            min_text = self._format_current_si(min(currents))
            max_text = self._format_current_si(max(currents))
            self.result_var.set(
                f"{len(currents)} samples | min {min_text} | max {max_text}"
            )
            self._log(f"Result summary: min {min_text}, max {max_text}")
            self._log("Captured currents (SI):")
            for idx, value in data:
                self._log(f"  {idx:>4d}: {self._format_current_si(value)}")
    ...
    def _parse_measurements(self, lines: list[str]) -> tuple[list[tuple[int, float]], str | None]:
        ...
        for line in lines:
            ...
            if line.lower().startswith("error"):
                return [], line
            if "reading" in line.lower() and "current" in line.lower():
                continue
    ...
    def _update_plot(self, data: list[tuple[int, float]]) -> None:
        ...
        scale_factor, y_label = self._determine_axis_scale(data)
        self.ax.set_ylabel(f"Current ({y_label})")
        self.ax.grid(True, linestyle="--", alpha=0.6)
        if data:
            x_vals = [idx for idx, _ in data]
            y_vals_raw = [val for _, val in data]
            y_vals = [val / scale_factor for val in y_vals_raw]
            self.ax.plot(x_vals, y_vals, marker="o", markersize=4, linewidth=1.4, color="tab:orange")
            self.ax.fill_between(x_vals, y_vals, color="tab:orange", alpha=0.2)
            self.ax.set_xlim(min(x_vals) - 0.5, max(x_vals) + 0.5)
            y_min = min(y_vals)
            y_max = max(y_vals)
            margin = max(1e-9, (y_max - y_min) * 0.1)
    ...
    def _determine_axis_scale(self, data: list[tuple[int, float]]) -> tuple[float, str]:
        """Return (scale_factor, unit_label) for plotting."""
        if not data:
            return 1.0, "A"
        values = [abs(v) for _, v in data]
    ...
    def _format_current_si(self, value: float) -> str:
        """Render current with an appropriate SI prefix (A/mA/uA/nA)."""
        magnitude = abs(value)
        if magnitude == 0:
            return "0 A"
        thresholds = [
        ...
        # Smaller than the last threshold: report in pA for readability
        return f"{value / 1e-12:.6g} pA"
...
def main() -> None:
    root = tk.Tk()
    ExternalTriggerCurrentGUI(root)
    root.mainloop()
